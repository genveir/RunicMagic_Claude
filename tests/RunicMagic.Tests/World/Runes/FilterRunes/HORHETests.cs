...
namespace RunicMagic.Tests.World.Runes.FilterRunes;
...
public class HORHETests
{
    ...
    {
        var left = new EntityBuilder().WithLocation(x: -200, y: 0).Build();
        var right = new EntityBuilder().WithLocation(x: 200, y: 0).Build();
        var far = new EntityBuilder().WithLocation(x: 500, y: 0).Build();
