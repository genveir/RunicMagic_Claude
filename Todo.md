...
| RMC-70 | Tax peak selection breadth during set resolution | Change `IEntitySet.Resolve()` to return an `EntitySetResolution` carrying both the final `EntitySet` and a `HashSet<EntityId>` of every entity ever touched by a leaf selector during resolution. Leaf selectors populate the set with all resolved entity IDs. Unary filters pass it through unchanged. Expansion selectors (LA) add their discovered entities to the inner set's HashSet. Binary set operations (union, intersection, difference) union both inputs' HashSets. `EntitySetSelectionCostResolver` charges the existing `ceil(MaxPower/1000)` per entity in the final set plus an additional `floor(touchedCount / 10)` flat cost. A precise spell that selects exactly what it needs pays less than one that sweeps broadly and filters back down. | |
